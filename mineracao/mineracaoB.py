...
def carregar_rotulos(fits_file):
    ...
    with fits.open(fits_file) as hdul:
        ...
        # Cria um dicionário para os rótulos
        rotulos = {}

        for i in range(len(data)):
            nome_imagem = data[i]['filename']  # Obtém o nome da imagem

            # Define os rótulos com base nos valores das colunas
            if data[i]['SPIRAL'] == 1:
...
def carregar_imagem(caminho_imagem, colorido=False):
    ...
    if caminho_imagem.endswith('.fits'):
        ...
        with fits.open(caminho_imagem) as hdul:
            ...
            # Se precisar de uma imagem colorida
            if colorido:
                # Converte para RGB, assumindo que a imagem é grayscale
                img_data = cv2.cvtColor(img_data, cv2.COLOR_GRAY2BGR)

            return img_data
    else:
        # Se não for FITS, converte para FITS
        try:
            img_array = plt.imread(caminho_imagem)

            # Converte o array NumPy para um objeto FITS
            hdu = fits.PrimaryHDU(img_array)  
            
            # Cria um header FITS básico
            hdr = fits.Header()
            hdu.header = hdr

            img = hdu  # Atribui o objeto FITS à variável img
...
def prever_classes(modelo, diretorio_imagens, rotulos_verdadeiros):
    ...
    # Limiar de confiança para prever uma classe
    ...
    for imagem_nome in os.listdir(diretorio_imagens):
        ...
        img = carregar_imagem(caminho_imagem)  # Carrega a imagem

        if img is not None:
            # Converte a imagem para FITS, se necessário
            ...
            # Verifica se a imagem é colorida (3 canais) ou não (1 canal)
            if img_rgb.ndim == 3 and img_rgb.shape[2] == 3:
                # Para imagens RGB, o corte pode ser feito manualmente sem usar Cutout2D
                # Faz o corte diretamente na imagem RGB
                y_center, x_center = img_rgb.shape[0] // 2, img_rgb.shape[1] // 2
                cutout = img_rgb[y_center - 32:y_center + 32, x_center - 32:x_center + 32]  # Exemplo de corte 64x64
            else:
                # Caso a imagem seja em escala de cinza ou outro formato, trata-se sem o Cutout2D
            ...
            probabilidade_maxima = np.max(pred)  # Probabilidade da classe mais provável
    ...
    for i, (img, pred, nome_imagem) in enumerate(zip(imagens, predicoes, nomes_imagens)):
        ...
        ax.imshow(img, cmap='gray')  # Ajustado para exibir imagens em escala de cinza
...
def plotar_grafico_brilho(brilhos_observados, previsoes_brilho):
    ...
    for i in range(n_bandas):
        plt.plot(range(len(brilhos_observados), len(brilhos_observados) + len(previsoes_brilho)), 
                 ...
                 color=cores[i], linestyle='--', marker='o', markersize=3)  # Adiciona marcadores
